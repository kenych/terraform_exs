...
while [ "None" = "$(aws ssm get-parameters --names 'k8s-init-token' --query '[Parameters[0].Value]' --output text  --with-decryption--region eu-west-2 2> /dev/null)" ];do echo "waiting for init master"; sleep 5;done
